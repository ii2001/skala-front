...
- `formEnhancements.js`: 회원가입 단계 안내, 비밀번호 확인·표시 전환, 필드별 실시간 검증과 자기소개 글자 수 표시
...
- `html/signUp.html`: 입력 단계 안내, 비밀번호 확인과 필드별 실시간 검증을 적용한 회원가입 페이지
...
- 회원가입 5단계 바로가기와 진행 상태, 비밀번호 확인·표시 전환, 입력별 성공·오류 안내 및 300자 자기소개 카운터
...
저장소의 HTML 구조, 스타일 연결, 스크립트 경로와 위 기능의 구현 코드를 정적으로 대조했습니다. Headless Chrome과 로컬 서버로 데스크톱·모바일 렌더링, 회원가입 필드별 검증 안내, 주차 클릭·키보드 이동에 따른 일정표 갱신을 스모크 테스트했습니다. 다음 항목은 제출 전 대상 브라우저에서 실제 사용자 조작으로 다시 확인해야 합니다.

- 라이트·다크 테마 전환, 새로고침 후 선택 유지와 운영체제 테마 변경 반영
- 스크롤 reveal, 키보드 포커스와 운영체제의 모션 줄이기 설정
- 주차 카드 클릭·가로 스크롤·마우스 드래그·키보드 탐색, 현재 날짜와 일정표 갱신
- 회원가입 단계 이동, 필드별 실시간 성공·오류 안내, 제출 시 첫 오류 이동, 비밀번호 확인·표시 전환 및 글자 수 갱신
