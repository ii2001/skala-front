...
- `classScheduleData.js`와 `classSchedule.js`: 4기 4반 22주 교육 일정 데이터와 주차 카드·가로 스크롤·키보드 탐색
- `formEnhancements.js`: 회원가입 단계 안내, 비밀번호 표시·숨기기와 자기소개 글자 수 표시
...
- `html/signUp.html`: 입력 단계 안내와 브라우저 기본 유효성 검사를 적용한 회원가입 UI 데모
...
- 브라우저의 현재 날짜 강조와 주차 카드 클릭·가로 스크롤·방향키·Home·End 탐색
- 회원가입 5단계 바로가기와 진행 상태, 비밀번호 표시·숨기기 및 300자 자기소개 카운터
...
저장소의 HTML 구조, 스타일 연결, 스크립트 경로와 위 기능의 구현 코드는 정적으로 대조했습니다. 다음 항목은 Live Server로 실행한 뒤 대상 브라우저에서 직접 확인해야 합니다.

- 라이트·다크 테마 전환, 새로고침 후 선택 유지와 운영체제 테마 변경 반영
- 스크롤 reveal, 키보드 포커스와 운영체제의 모션 줄이기 설정
- 주차 카드 클릭·가로 스크롤·키보드 탐색, 현재 날짜와 일정표 갱신
- 회원가입 단계 이동, 브라우저 기본 유효성 검사, 비밀번호 표시와 글자 수 갱신
